...
/******************************************** 추가 ********************************************/
var td_id;		//테이블제목의 td id값	 ex)td0-1 
/********************************************************************************************/
...
function calendar_set(e)
{
    ...
    calendar_div.style.cssText = "width:260px; background-color:#ffffff; border:3px; border-style:double; border-color:#c2c2c2;";
...
function calendar_set_date(set_date)
{
    ...
    if(calendar_obj)
    {
        ...
        /**************************************************************** 추가 *********************************************************/
        var text = calendar_obj.value;					//YYYY-MM-DD
        $(td_id).text(text.substr(5)+" ");				//MM-DD
        var col = td_id.substr(td_id.indexOf("-")+1);	//행 숫자
        $("<input type='hidden' id='start_date"+col+"' name='start_date"+col+"' value='"+ text +"'>").appendTo(td_id);
        $("<input type='button' name='달력' value='▼' onClick=\"calendar(event, 'start_date"+col+"', '"+td_id+"');\">").appendTo(td_id);
        /*****************************************************************************************************************************/
    }
    calendar_close();
...
/**********************************************calendar()인자에 td_td 추가 **************************************/
function calendar(e, obj_name, id, fmt)
{
	td_id = id;										/************************** 추가 *************************/
    if(!e) var e = window.event;
...
/******************************************************* 추가 ****************************************************/
//출석,지각,결석,가산,삭제 버튼
function check_atttendance(status_kor, status_eng, status_val){
	console.log("***테이블제목 아이디:"+td_id);
	
	//체크박스
	var check  = $("input:checkbox[name=attendance-check]");
	//체크된 갯수
	var num_check = $("input:checkbox[name=attendance-check]:checked").length;
	...
	//td_id: '#td0-N' , col_number: '-N'
	col_number = td_id.substr(4);

	//체크박스 전체 순회 (테이블 최상단 checkbox 제외)
	for(var i=1 ; i<check.length ; i++){
		//체크됨
		if($(check[i]).is(":checked")) {
			//체크된 td id. td_check: '#tdN-N'
			var td_check = "#td" + check[i].value + col_number;
			var input_check = "input" + check[i].value + col_number;
			
			//해당 열의 빈칸일 때, '출석/지각/결석/가산'으로 변경
			if($(td_check).val() == -2){
				$(td_check).addClass(status_eng);
				$(td_check).text(status_kor);
				$(td_check).append("<input type='hidden' id='"+input_check+"' name='"+input_check+"' value='"+status_kor+"'/>");
				
				if(status_val != -2){
					//삭제 버튼 아닐 떄, 계산 함수 호출
					$(td_check).val(status_val); 					
					console.log("처음:내용값:"+$(td_check).val()+"버튼값:"+status_val);
					calculate(td_check, status_val, 0);				
				}
			}
			else if(($(td_check).val() != -2) && ($(td_check).val() == status_val)){
			//해당 열에 내용이 있고, 내용과 같은 버튼이 클릭됨
				//내용 삭제
				console.log("처음:내용값:"+$(td_check).val()+"버튼값:"+status_val);
				if(status_val != -2)
					calculate(td_check, status_val, 1);			//계산 함수 호출
				$(td_check).removeClass($(td_check).attr("class"));
				$(td_check).text("");
				$(td_check).append("<input type='hidden' id='"+input_check+"' name='"+input_check+"' value=''/>");
				$(td_check).val(-2);
			}
			else if(($(td_check).val() != -2) && ($(td_check).val() != status_val)){
			//해당 열에 내용이 있고, 내용과 다른 버튼이 클릭됨
				
				//존재하던 내용 삭제
				console.log("처음:내용값:"+$(td_check).val()+"버튼값:"+status_val);
				calculate(td_check, $(td_check).val(), 1);		//계산 함수 호출
				$(td_check).removeClass($(td_check).attr("class"));
				$(td_check).text("");
				$(td_check).val(-2);
...
//점수 계산		3~7열 : 잔여, 출석(0), 지각(-0.5), 결석(-1), 가산(+1), 삭제(-2)   
function calculate(td_check, btn_val, is_remove){
	var row = td_check.substring(3,td_check.indexOf("-"));		//선택된 행
	console.log("선택된 행:"+row);
	
	var rest = "#td"+row+"-3";								//선택된 행의 잔여점수 td의 id
	var input_rest = "input"+row+"-3";						//선택된 행의 input id
	var present = "#td"+row+"-4";							//선택된 행의 출석점수 td의 id
	var input_present = "input"+row+"-4";					//선택된 행의 input id
	var late = "#td"+row+"-5";								//선택된 행의 지각점수 td의 id
	var input_late = "input"+row+"-5";						//선택된 행의 input id
	var absent = "#td"+row+"-6";							//선택된 행의 결석점수 td의 id
	var input_absent = "input"+row+"-6";					//선택된 행의 input id
	var plus = "#td"+row+"-7";								//선택된 행의 가산점수 td의 id
	var input_plus = "input"+row+"-7";						//선택된 행의 input id
	
	
	//내용 생성
	if(is_remove == 0){
		//잔여점수 계산
		console.log("rest.val():"+$(rest).val()+"btn_val:"+btn_val);
		
		$(rest).val(parseFloat($(rest).val()) + parseFloat(btn_val));	
		$(rest).text($(rest).val());
		$(rest).append("<input type='hidden' id='"+input_rest+"' name='"+input_rest+"' value='"+$(rest).val()+"'/>");
		console.log("+내용값:"+$(td_check).val()+"+잔여:"+$("#"+input_rest).val());
		
		//각 버튼의 값에 따라 점수 계산 
		switch(parseFloat(btn_val)){
		case 0:
			//출석 버튼, 출석 횟수		
			$(present).val(parseInt($(present).val()) + 1);
			$(present).text($(present).val()); 
			$(present).append("<input type='hidden' id='"+input_present+"' name='"+input_present+"' value='"+$(present).val()+"'/>");
			break;
		case -0.5:
			//지각 버튼, 지각 횟수
			$(late).val(parseInt($(late).val()) + 1);
			$(late).text($(late).val());
			$(late).append("<input type='hidden' id='"+input_late+"' name='"+input_late+"' value='"+$(late).val()+"'/>");
			break;
		case -1:
			//결석 버튼, 결석 횟수
			$(absent).val(parseInt($(absent).val()) + 1);
			$(absent).text($(absent).val());
			$(absent).append("<input type='hidden' id='"+input_absent+"' name='"+input_absent+"' value='"+$(absent).val()+"'/>");
			break;
		case 1:
			//가산 버튼, 가산 횟수
			$(plus).val(parseInt($(plus).val()) + 1);
			$(plus).text($(plus).val());
			$(plus).append("<input type='hidden' id='"+input_plus+"' name='"+input_plus+"' value='"+$(plus).val()+"'/>");
			break;
		default:
			break;
		}
